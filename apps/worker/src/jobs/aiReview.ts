...
export async function aiReview(payload: AiReviewPayload, deps: AiDeps = {}, delivery?: AiReviewDelivery): Promise<void> {
  const { reportId } = payload;
  const svc = getServiceClient();

  const { data: report, error: reportError } = await svc
    .from('reports')
    .select('spec_id, variant, extracted_data, operator_overrides, extraction_issues, status, data_revision, ai_review')
    .eq('id', reportId)
    .maybeSingle();
  if (reportError) throw new RetryableAiReviewError('Não foi possível consultar a revisão de IA.');
  const r = report as {
    spec_id: string;
    variant: string | null;
    ...
    ai_review: { status?: string; runId?: string; executionId?: string; jobId?: string; attempt?: number; data?: Record<string, FieldValue>; dependencies?: Record<string, string[]> } | null;
    status: string;
  } | null;
  ...
  if (payload.dataRevision !== undefined && payload.dataRevision !== r.data_revision) return;
  if (payload.runId && (r.ai_review?.runId !== payload.runId || !['queued', 'running'].includes(r.ai_review.status ?? ''))) return;
  if (delivery && r.ai_review?.status === 'running' && r.ai_review.jobId) {
    ...
    if (r.ai_review.jobId !== delivery.jobId || (r.ai_review.attempt ?? -1) >= delivery.attempt) return;
  }
  ...
  const baseState = { data: r.ai_review?.data, dependencies: r.ai_review?.dependencies, runId, executionId, revision: r.data_revision,
    jobId: delivery?.jobId, attempt: delivery?.attempt };
  const claim = svc.from('reports').update({ ai_review: { ...baseState, status: isAiEnabled() ? 'running' : 'disabled', updatedAt: new Date().toISOString() } } as never)
    .eq('id', reportId).eq('data_revision', r.data_revision).in('status', ['extracted', 'in_review']);
  if (payload.runId) {
    claim.eq('ai_review->>runId', runId).eq('ai_review->>status', r.ai_review!.status!);
    if (r.ai_review?.executionId) claim.eq('ai_review->>executionId', r.ai_review.executionId);
    else claim.is('ai_review->>executionId', null);
  }
  const { data: claimed, error: claimError } = await claim.select('id').maybeSingle();
  if (claimError) throw new RetryableAiReviewError('Não foi possível registrar a análise de IA.');
  if (!claimed || !isAiEnabled()) return;

  const persist = async (patch: Record<string, unknown>) => {
    try {
      const result = await svc.from('reports').update(patch as never)
        .eq('id', reportId).eq('data_revision', r.data_revision).eq('ai_review->>runId', runId)
        .eq('ai_review->>executionId', executionId).eq('ai_review->>status', 'running')
        .in('status', ['extracted', 'in_review']).select('id').maybeSingle();
      if (result.error) throw result.error;
      return result;
    } catch { throw new RetryableAiReviewError('Não foi possível persistir a revisão de IA.'); }
  };
  ...
  const { data: specRow, error: specError } = await svc.from('report_specs').select('spec').eq('id', r.spec_id).maybeSingle();
  if (specError) throw new RetryableAiReviewError('Não foi possível consultar o spec para a revisão de IA.');
  try {
    ...
    for (const [name, def] of collectFields(spec, r.variant)) {
      ...
      effective[name] = resolveFieldValue(name, r.operator_overrides ?? {}, r.extracted_data);
    }
    const valid = new Set(cells.keys());
    const { system, userText } = buildReviewPrompt(spec, r.variant, effective);
    const text = await callLLM({ purpose: 'ai_review', reportId, system, content: [{ type: 'text', text: userText }], maxTokens: 2048 }, deps);
    ...
    if (warnings.some((w: unknown) => !w || typeof w !== 'object' || typeof (w as AiWarning).field !== 'string' || typeof (w as AiWarning).message !== 'string')) throw new Error('invalid_response');
    const aiIssues = aiWarningsToIssues(warnings, valid, (f) => cells.get(f) ?? null);
    const dependencies: Record<string, string[]> = {};
    for (const w of warnings as (AiWarning & { related_fields?: unknown })[]) {
      if (!valid.has(w.field)) continue;
      const fields = Array.isArray(w.related_fields) && w.related_fields.every((f) => typeof f === 'string' && valid.has(f))
        ? w.related_fields as string[] : [...valid]; // sem proveniência: invalidação conservadora
      dependencies[w.field] = [...new Set([w.field, ...(dependencies[w.field] ?? []), ...fields])];
    }
    const kept = (r.extraction_issues ?? []).filter((i) => i.origin !== 'ai');
    const { data: saved } = await persist({
      extraction_issues: [...kept, ...aiIssues],
      ai_review: { ...baseState, status: 'done', updatedAt: new Date().toISOString(), data: effective, dependencies },
    });
    if (!saved) return; // dados/etapa mudaram enquanto a IA respondia
    await svc.from('audit_log').insert({ report_id: reportId, actor: null, action: 'ai_review', payload: { warnings: aiIssues.length, revision: r.data_revision } } as never);
  } catch (error) {
    if (error instanceof RetryableAiReviewError) throw error;
    await persist({ ai_review: { ...baseState, status: 'error', updatedAt: new Date().toISOString(), error: 'A IA não conseguiu concluir a revisão. Tente novamente; o fluxo manual continua disponível.' } });
  }
}
