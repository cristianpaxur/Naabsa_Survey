...
vi.mock('../lib/soffice', () => ({
  convertDocxToPdf: convert,
  measureBookmarkPages: vi.fn(),
}));
  ...
  getServiceClient: () => ({
      ...
      from: () => ({
        download: async (path: string) => ({
          data: state.objects.has(path)
            ? { arrayBuffer: async () => state.objects.get(path) }
            : null,
          error: null,
        }),
        upload: async (
    ...
    from: () => {
      ...
      const q = {
        ...
        single: async () => ({
          data: filters.every(([k, v]) => state.report[k] === v)
            ? { ...state.report }
            : null,
          error: null,
        }),
        insert: async (event: { action: string }) => {
        ...
        then: (resolve: (value: unknown) => unknown) => {
          if (state.failUpdate)
            return Promise.resolve(
              resolve({ error: { message: 'DB offline' }, count: null }),
            );
...
beforeEach(() => {
  ...
  state.report = {
    ...
    status: 'approved',
    deleted_at: null,
    approved_docx_revision: 2,
    approved_docx_path: 'r1/saved-2.docx',
    pdf_paths: [],
    report_types: { slug: 'draft_survey' },
  };
  state.objects.set('r1/saved-2.docx', Buffer.from('MANUAL-2'));
  convert.mockImplementation(async (docx: Buffer) =>
    Buffer.from(`PDF:${docx.toString()}`),
  );
});
...
describe('generatePdf conserva a aprovação diante de retries e jobs atrasados', () => {
  it('duas execuções geram apenas um PDF referenciado sem reconstruir o DOCX', async () => {
    ...
    expect(state.objects.get('r1/final-v1-r2.pdf')?.toString()).toBe(
      'PDF:MANUAL-2',
    );
    expect(convert).toHaveBeenCalledOnce();
    expect(
      state.audits.filter((a) => a.action === 'pdf_generated'),
    ).toHaveLength(1);
  });
  ...
  it('mudança de ciclo durante conversão não publica PDF antigo nem ocupa nome do novo', async () => {
    convert.mockImplementationOnce(async () => {
      state.report.approved_docx_revision = 3;
      state.report.approved_docx_path = 'r1/saved-3.docx';
      state.objects.set('r1/saved-3.docx', Buffer.from('MANUAL-3'));
      return Buffer.from('OLD PDF');
    });
    await generatePdf({ reportId: 'r1', approvedRevision: 2 });
    expect(state.report.status).toBe('approved');
    expect(state.report.pdf_paths).toEqual([]);
    await generatePdf({ reportId: 'r1', approvedRevision: 3 });
    expect(state.report.pdf_paths).toEqual(['r1/final-v1-r3.pdf']);
    expect(state.objects.get('r1/final-v1-r3.pdf')?.toString()).toBe(
      'PDF:MANUAL-3',
    );
  });
  it('retry após upload e falha DB reaproveita objeto sem duplicar versão', async () => {
    ...
    await expect(
      generatePdf({ reportId: 'r1', approvedRevision: 2 }),
